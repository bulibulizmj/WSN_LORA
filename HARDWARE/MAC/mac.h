...
/* CSMA/CA congestion measurement window (number of CSMA listen windows). */
#define MAC_CSMA_CONGESTION_WINDOW_SIZE       64u
...
typedef struct {
    ...
    u8 require_ack;                       //是否需要ACK，仅数据型MAC帧设置时有效，如果require_ack=1表示当前通信结束，对方需要发送ACK，为0表示还需要对方传输数据型MAC帧
...
void mac_frame_clear(MACframe* macframe);							      //清空结构体缓冲
