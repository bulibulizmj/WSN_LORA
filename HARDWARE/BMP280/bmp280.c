...
static s32 bmp280RawPressure = 0;//s32是32位整型
...
bool bmp280Init(void)
{
	...
	for (i=0; i<10; i++)
	{
		id=IIC_Read_One_Byte(BMP280_I2C_ADDR,BMP280_CHIP_ID);//读取ID
//		printf("id = %x\n",id);
	...
	{
		IIC_Read_Len(BMP280_I2C_ADDR,BMP280_TEMPERATURE_CALIB_DIG_T1_LSB_REG,24,(u8*)&bmp280Cal);//读取校准数据
		IIC_Write_One_Byte(BMP280_I2C_ADDR,BMP280_CTRL_MEAS_REG,BMP280_MODE);//设置过采样率和工作模式
		IIC_Write_One_Byte(BMP280_I2C_ADDR,BMP280_CONFIG_REG,BMP280_FILTER);//配置IIR滤波
...
//		i2cdevRead(I2C1_DEV, BMP280_I2C_ADDR, BMP280_PRESSURE_MSB_REG, BMP280_DATA_FRAME_SIZE, data);
		...
		IIC_Read_Len(BMP280_I2C_ADDR,BMP280_PRESSURE_MSB_REG,BMP280_DATA_FRAME_SIZE,data);//连续读六个地址
...
uint32_t bmp280GetRawData(void)
{
	bmp280GetPressure();
	
  bmp280CompensateT(bmp280RawTemperature);	      /*单位度*/	
	return bmp280CompensateP(bmp280RawPressure);		/*单位Pa*/	
